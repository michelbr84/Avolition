...
import json
import os
from src.player.chargen import CharGen
from src.audio.soundpool import SoundPool
from src.core.engine import Spawner, LevelLoader
...
class Game(DirectObject):
    def __init__(self, loadscreen, root):
        ...
        self.common["key_icon"]=DirectFrame(frameSize=(-64, 0, 0, 64),
                                    frameColor=(1, 1, 1, 1),
                                    frameTexture="assets/icons/items/icon_key2.png",
                                    parent=pixel2d)
        ...
        if not config_safemode.getValue():
            if config_bloom.getValue():
                ...
                glowShader=loader.loadShader("assets/shaders/glow.sha")
                tempnode = NodePath(PandaNode("temp node"))
                tempnode.setShader(glowShader)
                self.bloomCamera.node().setInitialState(tempnode.getState())
                self.blurBuffer=self.makeFilterBuffer(self.bloomBuffer,  "Blur X", -2, "assets/shaders/blur.sha")
            ...
            quad.setShader(Shader.load("assets/shaders/shadow.sha"))
